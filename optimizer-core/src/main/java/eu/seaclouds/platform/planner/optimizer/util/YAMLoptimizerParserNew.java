...
public class YAMLoptimizerParserNew {

    // Reducing verbosity . If somebody knows a better approach for doing this
    // (I
    ...
    static Logger log = LoggerFactory.getLogger(YAMLoptimizerParserNew.class);
    ...
    private static List<String> GetListOfSuitableOptionsForModule(
        ...
        // FOR EACH OF THE APP MODULES (but in this level there are more
        // concepts
        // than only the modules)
        for (Map.Entry<String, Object> entry : appMap.entrySet()) {

            List<String> options = FindSuitableOptionsForEntry(modulename,
                    entry);
            if (options != null) {
                return options;
            }
    ...
    private static List<String> FindSuitableOptionsForEntry(String modulename,
        ...
        if (modulename.equals(entry.getKey())) {
            ...
            return GetListOfSuitableOptionsForAlreadyFoundModule(entry
                    .getValue());
        }
        ...
        // come deeper in the tree)
            ...
            List<String> options = GetListOfSuitableOptionsForModule(
    ...
    @SuppressWarnings("unchecked")
    public static List<String> GetListOfSuitableOptionsForAlreadyFoundModule(
            Object appSubMap) {
        Map<String, Object> moduleInfo = null;
        try {
            moduleInfo = (Map<String, Object>) appSubMap;
        } catch (ClassCastException E) {
            // If it was not a Map, nothing to do, we are not in the correct
            // part
            // of the model.
            return null;
        }
        ...
        if (moduleInfo.containsKey(TOSCAkeywords.SUITABLE_SERVICES)) {
            return (List<String>) moduleInfo
                    .get(TOSCAkeywords.SUITABLE_SERVICES);
    ...
    @SuppressWarnings("unchecked")
    public static List<Module> getModuleListWithCharacteristics(
            Map<String, Object> appMap, Map<String, Object> allCloudOffersMAP) {

        Map<String, Object> modulesMap = (Map<String, Object>) appMap
                .get(TOSCAkeywords.NODE_TEMPLATE);

        List<Module> moduleList = new ArrayList<Module>();
        for (Map.Entry<String, Object> entry : modulesMap.entrySet()) {
            moduleList.add(new Module(entry, allCloudOffersMAP, modulesMap));
        }

        return moduleList;
    }

    public static double getCloudLatency(String latencyKeyword) {

        try {
            @SuppressWarnings("unchecked")
            Map<String, Object> cloudMap = Parameters.getAllCloudOffersMAP();
    ...
    public static double getCloudLatency(String latencyKeyword,
            Map<String, Object> allCloudOffersMAP) {

        try {
            @SuppressWarnings("unchecked")
            Map<String, Object> cloudMap = allCloudOffersMAP;
    ...
    public static List<CloudOffer> getCloudOfferCharacteristcisByName(
            List<String> potentialListOfOffers,
    ...
    private static CloudOffer getAllCharacteristicsOfCloudOffer(
            String potentialOffer, Map<String, Object> cloudOffersNewMap) {

        Map<String, Object> cloudMap = cloudOffersNewMap;

        if (cloudOffersNewMap.containsKey(TOSCAkeywords.NODE_TEMPLATE)) {
            cloudMap = (Map<String, Object>) cloudOffersNewMap
                    .get(TOSCAkeywords.NODE_TEMPLATE);
        }
        ...
        try {
            ...
            offer.setType(YAMLoptimizerParserNew.getTypeOfCloudOffer(
                  TOSCAkeywords.CLOUD_OFFER_TYPE_TAG,
                  (Map<String, Object>) cloudMap.get(potentialOffer)));
            
            offer.setPerformance(YAMLoptimizerParserNew.getPropertyOfCloudOffer(
                  TOSCAkeywords.CLOUD_OFFER_PROPERTY_PERFORMANCE,
                  (Map<String, Object>) cloudMap.get(potentialOffer)));
            offer.setAvailability(YAMLoptimizerParserNew.getPropertyOfCloudOffer(
                  TOSCAkeywords.CLOUD_OFFER_PROPERTY_AVAILABILITY,
                  (Map<String, Object>) cloudMap.get(potentialOffer)));
            offer.setCost(YAMLoptimizerParserNew.getPropertyOfCloudOffer(
                  TOSCAkeywords.CLOUD_OFFER_PROPERTY_COST,
                  (Map<String, Object>) cloudMap.get(potentialOffer)));
            offer.setNumCores(YAMLoptimizerParserNew.getPropertyOfCloudOffer(
    ...
    public static String getTypeOfCloudOffer(String cloudOfferType,
          Map<String, Object> singleOfferMap) {
       
      String valueOfType = (String) singleOfferMap.get(cloudOfferType);
      
      return valueOfType;
  }

    public static double getPropertyOfCloudOffer(String cloudOfferProperty,
            Map<String, Object> singleOfferMap) {
        ...
        if (propertiesOfOffer.containsKey(cloudOfferProperty)) {
            ...
            if (BeeingTooVerboseWithLackOfInformationInCloudOffers > 0) {
                log.info("Property "
                        ...
                        + cloudOfferProperty
                        + " requirement existed in the system");
    ...
    public static void AddSuitableOfferForModule(String moduleName,
            String solutionName, int instances,
    ...
    public static Map<String, Object> getMAPofCloudOffers(
            String cloudOfferString) {
        Yaml yamlApp = new Yaml();
        @SuppressWarnings("unchecked")
    ...
    public static Topology getApplicationTopology(Map<String, Object> appMap,
        ...
        // gets the topology of the connected graph to element passed as
        // argument.
    ...
    private static Map.Entry<String, Object> getInitialElement(
            Map<String, Object> appMap) {
        // We assume that The initial element is such one that is not required
        // by
        // anyone.
...
//            if (YAMLmodulesOptimizerParserNew.ModuleHasModuleRequirements(
//                    entry.getValue(), appMap)
//                    && (!moduleIsRequiredByOthers(appMap, potentialModuleName))) {
//                return entry;
//            }
            if (!moduleIsRequiredByOthers(appMap, potentialModuleName)) {
               return entry;
            }
        }
    ...
    private static boolean moduleIsRequiredByOthers(Map<String, Object> appMap,
        ...
        for (Map.Entry<String, Object> entry : appMap.entrySet()) {
            if (YAMLmodulesOptimizerParserNew.ModuleRequirementFromTo(
    ...
    private static Topology getApplicationTopologyRecursive(String elementName,
            Map<String, Object> elementInternalMAP, Topology topology,
        ...
        double hostPerformance = getPerformanceOfOfferByName(
                YAMLmodulesOptimizerParserNew
                        .getMeasuredPerformanceHost(elementInternalMAP),
                allCloudOffersMAP);
        newelement.setExecTimeMillis(YAMLmodulesOptimizerParserNew
                .getMeasuredExecTimeMillis(elementInternalMAP)
                * hostPerformance);

        // The module does not have requiremetns
        if (!YAMLmodulesOptimizerParserNew.ModuleHasModuleRequirements(
                elementInternalMAP, modulesMAP)) {
            // Include it directly
        ...
        for (String moduleReqName : YAMLmodulesOptimizerParserNew
                .ModuleRequirementsOfAModule(elementInternalMAP, modulesMAP)) {
            // For each requiremnt of teh element (that is not its host but it's
            // a
            ...
            if (topology.contains(moduleReqName)) {
                ...
                double opProfileBetweenModules = YAMLmodulesOptimizerParserNew
                        .getOpProfileWithModule(elementInternalMAP,
                                moduleReqName);
                ...
                topology = getApplicationTopologyRecursive(moduleReqName,
                        (Map<String, Object>) modulesMAP.get(moduleReqName),
                        topology, modulesMAP, allCloudOffersMAP);
                double opProfileBetweenModules = YAMLmodulesOptimizerParserNew
                        .getOpProfileWithModule(elementInternalMAP,
                                moduleReqName);
                // create the dependence between these two modules by
    ...
    private static String getFinalHostNameOfModule(Map<String, Object> modules,
        ...
        if (modules.containsKey(YAMLmodulesOptimizerParserNew
                .getHostOfModule(module))) {
            return getFinalHostNameOfModule(modules,
                    YAMLmodulesOptimizerParserNew.getHostOfModule(module));
        } else {
            return modName;
        }
    ...
    private static void thresholdsFromSecondsToMinutes(
        ...
        for (Map.Entry<String, ArrayList<Double>> entry : thresholds.entrySet()) {
            ...
            // multiply each element by the ratio between seconds and minutes
            // (60)
    ...
    public static Map<String, Object> cloneYAML(Map<String, Object> yamlMap) {
        ...
        String stringyaml = YAMLoptimizerParserNew.FromMAPtoYAMLstring(yamlMap);
        Map<String, Object> newMap = YAMLoptimizerParserNew
                .GetMAPofAPP(stringyaml);
