...
public class SinglePointCrossover extends Crossover {
  ...
  private static final List VALID_TYPES = Arrays.asList(BinarySolutionType.class,
  		                                            ...
  		                                            IntSolutionType.class, SeaCloudsType.class) ;
    ...
    try {
    	...
    	if (parent1.getType().getClass() == SeaCloudsType.class) {
            ...
            int numOfModules = ((Modules) parent1.getDecisionVariables()[0])
                  .getModuleList().size();
            int crosspoint = PseudoRandom.randInt(0, numOfModules - 1);

            // Try a Random probability to decide whether to compute the Crossover or not
            if (PseudoRandom.randDouble() < probability) {
               
               for (int i = crosspoint; i < numOfModules; i++) {
                  if (((Modules) parent1.getDecisionVariables()[0])
                        .getModuleList().get(i).isComputable()) {
                     
                     CloudOffer temp1 = ((Modules) parent1
                           .getDecisionVariables()[0]).getModuleList().get(i)
                           .getBestCloudOffer();
                     CloudOffer temp2 = ((Modules) parent2
                           .getDecisionVariables()[0]).getModuleList().get(i)
                           .getBestCloudOffer();
                     ((Modules) offSpring[0].getDecisionVariables()[0])
                           .getModuleList().get(i).setBestCloudOffer(temp2);
                     ((Modules) offSpring[1].getDecisionVariables()[0])
                           .getModuleList().get(i).setBestCloudOffer(temp1);
                  }
               }
